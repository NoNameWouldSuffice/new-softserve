...
from softserve.mqttTopic import MqttTopic
import time
import ssl
import atexit
...
class Jackbord():
    def __init__(self, profileID):
        ...
        # Dictionary that stores channel class instances and mqttTopic class instances for later use
        self.__topicClassList = {}  # Format: "mqtt topic" : class instance
    ...
    def __openMqttServer(self):
        ...
        while self.__mqttResultCode == -1:
            ...
            if self.__mqttResultCode != 0 and self.__mqttResultCode != -1:
                resultMessage = mqtt.connack_string(self.__mqttResultCode)
                raise Exception("{0} Result code is {1}".format(resultMessage, self.__mqttResultCode))

    def __onMqttConnect(self, client, userdata, flags, rc):
        if len(self.__topicClassList.keys()) != 0:
            for topic in self.__topicClassList.keys():
    ...
    def __onMqttMessage(self, client, userdata, message):
        ...
        if (message.topic) in self.__topicClassList.keys():
            incomingValue = message.payload.decode()

            if incomingValue != self.__topicClassList[message.topic].get():
                self.__topicClassList[message.topic].updateFromServer(
                    incomingValue)
    ...
    def subscribe(self, mqttTopic):
        newTopicClass = MqttTopic(self.__mqttClient, self.__profileID, mqttTopic, self)

        self.__topicClassList[str(self.__profileID + "/" + mqttTopic)] = newTopicClass

        self.__mqttClient.subscribe(str(self.__profileID + "/" + mqttTopic))

        return(newTopicClass)
    
    def publish(self, mqttTopic, value):
        newMID = self.__mqttClient.publish(
            topic=str(self.jackbordID + "/" + mqttTopic), payload=value)[1]
        self.__updateSentMID(newMID)
